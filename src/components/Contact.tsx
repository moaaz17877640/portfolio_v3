  ...
  <motion.h2 initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-3xl md:text-4xl font-bold text-neutral-900 dark:text-white">Contact</motion.h2>
      <div className="mt-8 grid md:grid-cols-2 gap-6">
        <GlassCard className="p-6 md:p-8">
          <form action={`mailto:${profile.email}`} method="post" encType="text/plain" className="space-y-4">
            <div>
        ...
        <GlassCard className="p-6 md:p-8">
          <div className="grid gap-3 text-neutral-800 dark:text-white/80">
            <a className="flex items-center gap-3 hover:text-black dark:hover:text-white" href={`mailto:${profile.email}`}><Mail className="w-5 h-5" /> {profile.email}</a>
            <a className="flex items-center gap-3 hover:text-black dark:hover:text-white" href={`tel:${profile.phone}`}><Phone className="w-5 h-5" /> {profile.phone}</a>
            <a className="flex items-center gap-3 hover:text-black dark:hover:text-white" href={profile.github} target="_blank" rel="noreferrer"><Github className="w-5 h-5" /> GitHub</a>
            <a className="flex items-center gap-3 hover:text-black dark:hover:text-white" href={profile.linkedin} target="_blank" rel="noreferrer"><Linkedin className="w-5 h-5" /> LinkedIn</a>
