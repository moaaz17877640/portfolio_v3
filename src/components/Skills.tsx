        ...
        <div className="text-xs font-mono uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
          ...
          {activeStage && (
            <button
              ...
              className="ml-auto text-xs text-neon-cyan hover:underline capitalize"
            >
              Reset Stage Filter
            </button>
          )}
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-xl border text-left transition-all duration-200 ${
                  isSelected
                    ? 'bg-neon-cyan/15 border-neon-cyan text-white shadow-glow'
                    : 'bg-dark-900/60 border-white/[0.08] hover:border-white/20 text-slate-400 hover:text-white'
                }`}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className={`p-1.5 rounded-md ${isSelected ? 'bg-neon-cyan/20 text-cyan-300' : 'bg-white/[0.04] text-slate-400'}`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  <span className="text-[10px] font-mono text-slate-500">0{idx + 1}</span>
                </div>
                <div className="text-xs font-bold text-white tracking-tight">{stage.title}</div>
                <div className="text-[10px] text-slate-400 line-clamp-1 mt-0.5">{stage.description}</div>
              </button>
            )
          })}
        </div>
      ...
      <div className="flex items-center gap-2 mb-8 overflow-x-auto pb-2">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all shrink-0 ${
            activeCategory === 'all'
              ? 'bg-gradient-to-r from-neon-cyan to-indigo-500 text-white shadow-glow'
            ...
            return (
              ...
              >
                <SpotlightCard
                  ...
                  className="p-5 h-full flex flex-col justify-between border-white/[0.08] hover:border-white/20 group"
                >
                  <div>
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/[0.04] border border-white/[0.08] text-slate-400 capitalize">
                        {skill.stage}
                      </span>
                    </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-white/[0.06]">
                    <div className="flex items-center justify-between text-[11px] font-mono mb-1.5">
                      ...
                      <span className="text-slate-200 font-semibold">{skill.proficiency}%</span>
                    </div>
